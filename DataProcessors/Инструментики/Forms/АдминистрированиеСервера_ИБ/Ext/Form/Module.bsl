...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если ЭтоНовая Тогда
		...
		Элементы.ЗапретитьЛокальноеРаспознаваниеРечи.ТолькоПросмотр = Истина;
		Элементы.ГруппаПриОтсутствииАктивныхПользователей.ТолькоПросмотр = Истина;
	...
	Если Объект.ВерсияПлатформы < 3.24 Тогда
		Элементы.ЗапретитьЛокальноеРаспознаваниеРечи.Доступность = Ложь;
	КонецЕсли;
	
	Если Объект.ВерсияПлатформы < 3.25 Тогда
		Элементы.ГруппаПриОтсутствииАктивныхПользователей.Доступность = Ложь;
	КонецЕсли;
КонецПроцедуры
...
Функция ИменаСвойствДляЗаполненияПараметровСоединенияИБ()
	Возврат "Имя, Описание, ЗащищенноеСоединение, БлокировкаНачалаСеансов, БлокировкаРегламентныхЗаданий,
		...
		|ВнешнееУправлениеСеансами, ПрофильБезопасности, ПрофильБезопасностиБезопасногоРежима, ТипСУБД,
		|ИмяБазыСУБД, ЛогинСУБД, ПарольСУБД, СерверСУБД, НачалоБлокировкиСеансов, ОкончаниеБлокировкиСеансов,
		|КодРазрешения, ПараметрБлокировкиСеансов, СообщениеБлокировкиСеансов, КодЯзыка, СоздатьБазуДанных, СмещениеДат,
		|ЗапретитьЛокальноеРаспознаваниеРечи, ЗадержкаВыгрузкиКонфигурацииРабочимПроцессом,
		|МинимальныйПериодЗапускаРегламентныхЗаданий, МаксимальныйСдвигЗапускаРегламентныхЗаданий";
КонецФункции

&НаКлиенте
Асинх Процедура ЗапроситьЗакрытиеАсинх()
	ТекстВопроса = НСтр("ru='Есть несохраненные изменения. Вы уверены, что хотите закрыть форму?'");
