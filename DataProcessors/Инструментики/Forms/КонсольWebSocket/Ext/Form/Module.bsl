...
Процедура СообщенияВыбор(Элемент, ВыбраннаяСтрока, Поле, СтандартнаяОбработка)
	Если Поле.Имя = "СообщенияТекст" Тогда
		СтрСообщения = Элемент.ТекущиеДанные;
		Если СтрСообщения.ЭтоДвоичныеДанные Тогда
			Если ЗначениеЗаполнено(СтрСообщения.АдресДвоичныхДанных) Тогда
				ПараметрыДиалога = Новый ПараметрыДиалогаПолученияФайлов;
				ПолучитьФайлССервераАсинх(СтрСообщения.АдресДвоичныхДанных,
					НСтр("ru = 'Сообщение WebSocket'; en = 'WebSocket message'"), ПараметрыДиалога);
				//
			КонецЕсли;
		Иначе
...
Процедура УстановитьРазорватьСоединение(Команда)
	...
	Иначе
		...
		Исключение
			Если ИнформацияОбОшибке().Код = "ИнструментикиОшибкаДляПользователя" Тогда
...
Асинх Процедура ОтправитьФайл(Команда)
	...
	ДобавитьСообщение(ТипСообщенияИсходящееСообщение(), ДвоичныеДанные);
КонецПроцедуры

&НаКлиенте
Процедура ОчиститьСообщенияКоманда(Команда)
	АдресаДвоичныхДанных = Новый Массив;
	Для Каждого СтрСообщения Из Сообщения Цикл
		Если ЗначениеЗаполнено(СтрСообщения.АдресДвоичныхДанных) Тогда
			АдресаДвоичныхДанных.Добавить(СтрСообщения.АдресДвоичныхДанных);
		КонецЕсли;
	КонецЦикла;
	Если АдресаДвоичныхДанных.Количество() > 0 Тогда
		УдалитьМножествоИзВременногоХранилища(АдресаДвоичныхДанных);
	КонецЕсли;
...
Процедура УМ_Сохранение_ПриЗаполненииНастроекИнструментаПереопределяемый(ВерсияФормата, ИмяОбъекта,
	...
	ВерсияФормата = 6;
	ИмяОбъекта = "КонсольWebSocket_Данные";
	ИмяИнструмента = "Консоль WebSocket";
	КлючНастроек = "КонсольWebSocket";
...
Процедура УМ_Сохранение_ПриЗаполненииСохраняемогоОбъектаПереопределяемый(Фабрика, Пакет)
	Пакет.АдресСервера = АдресСервера;
	Пакет.Таймаут = Таймаут;
	Пакет.Логин = Логин;
	
	Для Каждого СтрЗаголовки Из Заголовки Цикл
		ЗаголовокXDTO = Фабрика.Создать(Фабрика.Тип("instrumentiki", "КонсольWebSocket_Заголовок"));
		ЗаголовокXDTO.Имя = СтрЗаголовки.Имя;
		ЗаголовокXDTO.Значение = СтрЗаголовки.Значение;
		
		ЗаголовокXDTO.Проверить();
		Пакет.Заголовки.Добавить(ЗаголовокXDTO);
	КонецЦикла;
КонецПроцедуры
...
Процедура УМ_Сохранение_ПриЧтенииОбъектаПереопределяемый(Пакет, ВерсияФормата)
	АдресСервера = Пакет.АдресСервера;
	Таймаут = Пакет.Таймаут;
	Логин = Пакет.Логин;
	
	Заголовки.Очистить();
	Для Каждого ЗаголовокXDTO Из Пакет.Заголовки Цикл
		СтрЗаголовки = Заголовки.Добавить();
		СтрЗаголовки.Имя = ЗаголовокXDTO.Имя;
		СтрЗаголовки.Значение = ЗаголовокXDTO.Значение;
	КонецЦикла;
КонецПроцедуры
...
Процедура ПриПолученииСообщенияПоWebSocket(Соединение, Сообщение) Экспорт
	...
	ДобавитьСообщение(ТипСообщенияВходящееСообщение(), Сообщение, Истина);
КонецПроцедуры
...
&НаКлиенте
Процедура ДобавитьСообщение(ТипСообщения, ТекстИлиДвоичныеДанные, ХранитьДвоичныеДанные = Ложь)
	...
	Если ТипЗнч(ТекстИлиДвоичныеДанные) = Тип("Строка") Тогда
		Сообщение.Текст = ТекстИлиДвоичныеДанные;
	ИначеЕсли ТипЗнч(ТекстИлиДвоичныеДанные) = Тип("ДвоичныеДанные") Тогда
		Сообщение.ЭтоДвоичныеДанные = Истина;
		Если ХранитьДвоичныеДанные Тогда
			Сообщение.АдресДвоичныхДанных = ПоместитьВоВременноеХранилище(ТекстИлиДвоичныеДанные, УникальныйИдентификатор);
		КонецЕсли;
	Иначе
		ВызватьИсключение "Недопустимый тип данных сообщения";
	КонецЕсли;
...
Процедура УстановитьСоединение()
	...
	Исключение
		#Если ВебКлиент Тогда
			ВызватьИсключение("Установка соединения невозможна в связи с ошибкой платформы",,
				"ИнструментикиОшибкаДляПользователя",, ИнформацияОбОшибке());
		#Иначе
			ВызватьИсключение;
		#КонецЕсли
	КонецПопытки;
	...
	Исключение
		ИдентификаторСоединения = Неопределено;
		ВызватьИсключение("Ошибка открытия WebSocket-соединения. Возможно, неверно указан адрес сервера
...
Процедура УстановитьУсловноеОформление()
	...
	УстановитьУсловноеОфрормлениеСообщениеСДвоичнымиДаннымиССохранением(УсловноеОформление);
	УстановитьУсловноеОфрормлениеСообщениеСДвоичнымиДаннымиБезСохранения(УсловноеОформление);
КонецПроцедуры

&НаСервереБезКонтекста
Процедура УстановитьУсловноеОфрормлениеСообщениеСДвоичнымиДаннымиССохранением(УсловноеОформление)
	...
	ЭлементУО.Оформление.УстановитьЗначениеПараметра("Текст",
		НСтр("ru = '<двоичные данные, нажмите дважды для сохранения в файл>';
			|en = '<binary data, double-click to save>'"));
	
	Отбор = ЭлементУО.Отбор.Элементы.Добавить(Тип("ЭлементОтбораКомпоновкиДанных"));
	Отбор.ЛевоеЗначение = Новый ПолеКомпоновкиДанных("Сообщения.ЭтоДвоичныеДанные");
	Отбор.ВидСравнения = ВидСравненияКомпоновкиДанных.Равно;
	Отбор.ПравоеЗначение = Истина;
	
	Отбор = ЭлементУО.Отбор.Элементы.Добавить(Тип("ЭлементОтбораКомпоновкиДанных"));
	Отбор.ЛевоеЗначение = Новый ПолеКомпоновкиДанных("Сообщения.АдресДвоичныхДанных");
	Отбор.ВидСравнения = ВидСравненияКомпоновкиДанных.Заполнено;
	
	Поле = ЭлементУО.Поля.Элементы.Добавить();
	Поле.Поле = Новый ПолеКомпоновкиДанных("СообщенияТекст");
КонецПроцедуры

&НаСервереБезКонтекста
Процедура УстановитьУсловноеОфрормлениеСообщениеСДвоичнымиДаннымиБезСохранения(УсловноеОформление)
	ЭлементУО = УсловноеОформление.Элементы.Добавить();
	
	ЭлементУО.Оформление.УстановитьЗначениеПараметра("ЦветТекста", WebЦвета.ТемноСерый);
	ЭлементУО.Оформление.УстановитьЗначениеПараметра("Текст", НСтр("ru = '<двоичные данные>'; en = '<binary data>'"));
...
&НаСервереБезКонтекста
Процедура УдалитьМножествоИзВременногоХранилища(Знач Адреса)
