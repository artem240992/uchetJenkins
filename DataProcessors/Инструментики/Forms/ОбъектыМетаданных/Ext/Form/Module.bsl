...
Асинх Процедура ДеревоФункцийВыбор(Элемент, ВыбраннаяСтрока, Поле, СтандартнаяОбработка)
	...
	ИначеЕсли ТД.Действие = "Последовательность" Тогда
		...
		ОткрытьФорму(Объект.ИмяМетаданныхОбработки + ".Форма.ДинамическийСписок", ПараметрыФормы,, ИмяТаблицы);
...
Асинх Процедура ЗаменитьПредопределенноеЗначение(Команда)
	...
	ОткрытьФорму(Объект.ИмяМетаданныхОбработки + ".Форма.ОбъектыМетаданных_ЗаменаПредопределенного", ПараметрыФормы);
КонецПроцедуры
