...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	СохраненныеПараметрыСоединенияСКластером = МодульОбъекта.СохраненнаяНастройка(
		"АдминистрированиеСервера", "ПараметрыСоединенияСКластером");
...
Процедура ПриОткрытии(Отказ)
	...
	#Если ВебКлиент Тогда
		// Код ниже реализует открытие формы авторизации в кластере вне события открытия формы
		// для обхода ошибки веб-клиента (форма авторизации открывается поверх формы списка инструментов)
		Доступность = Ложь;
		ПодключитьОбработчикОжидания("НачатьАвторизациюВКластере", 0.1, Истина);
	#Иначе
...
Процедура СписокИнформационныхБазПередНачаломДобавления(Элемент, Отказ, Копирование, Родитель, Группа, Параметр)
	...
	Иначе
		...
		Если СохраненныеДанныеСУБД <> Неопределено Тогда
			ЗаполнитьЗначенияСвойств(ОписаниеИБ, СохраненныеДанныеСУБД);
		КонецЕсли;
		...
		ОткрытьФорму(Объект.ИмяМетаданныхОбработки + ".Форма.АдминистрированиеСервера_ИБ",
...
Процедура СписокСеансовПередУдалением(Элемент, Отказ)
	...
	Сеансы = Новый Массив;
	Сеанс = Новый Структура("ИмяИБ, ИдентификаторИБ, НомерСеанса",
		СтрСеансы.ИмяИБ, СтрСеансы.ИдентификаторИБ, СтрСеансы.НомерСеанса);
	//
...
//  * ЗадержкаВыгрузкиКонфигурацииРабочимПроцессом - Число -
//  * МинимальныйПериодЗапускаРегламентныхЗаданий - Число -
//  * МаксимальныйСдвигЗапускаРегламентныхЗаданий - Число -
//
&НаКлиентеНаСервереБезКонтекста
Функция НовоеОписаниеИнформационнойБазы()
	...
	Результат.Вставить("ЗапретитьЛокальноеРаспознаваниеРечи", Ложь);
	Результат.Вставить("ЗадержкаВыгрузкиКонфигурацииРабочимПроцессом", 0);
	Результат.Вставить("МинимальныйПериодЗапускаРегламентныхЗаданий", 0);
	Результат.Вставить("МаксимальныйСдвигЗапускаРегламентныхЗаданий", 0);
...
Процедура НачатьАвторизованноеДействиеСИБ(ИмяИБ, Действие)
	...
	ОткрытьФорму(Объект.ИмяМетаданныхОбработки + ".Форма.АдминистрированиеСервера_АвторизацияВИБ",
		ПараметрыФормы,,,,, ОповещениеОЗавершении);
	//
КонецПроцедуры
...
Функция СоединениеСКластеромCOM(ПараметрыСоединенияСКластером, ДляВыполненияДействийСИБ)
	...
	ПортСтрокой = Формат(ПараметрыСоединенияСКластером.ПортАгента, "ЧГ=");
	ПолныйАдресАгента = СтрШаблон("tcp://%1:%2", ПараметрыСоединенияСКластером.АдресАгента, ПортСтрокой);
	...
	Для Каждого Кластер Из Агент.GetClusters() Цикл
		...
		Агент.Authenticate(Кластер,
			ПараметрыСоединенияСКластером.ЛогинКластера, ПараметрыСоединенияСКластером.ПарольКластера);
		//
		...
		Если ДляВыполненияДействийСИБ Тогда
			...
			СоединениеСРабочимПроцессом.AuthenticateAdmin(ПараметрыСоединенияСКластером.ЛогинКластера,
				ПараметрыСоединенияСКластером.ПарольКластера);
			//
...
Функция СоединениеСКластеромRAS(ПараметрыСоединенияСКластером, ДляВыполненияДействийСИБ)
	АдминистрированиеСервера = Новый АдминистрированиеСервера(ПараметрыСоединенияСКластером.АдресАгента,
		ПараметрыСоединенияСКластером.ПортАгента);
	//
	АдминистрированиеСервера.ВыполнитьАутентификацию(ПараметрыСоединенияСКластером.ЛогинАгента,
		ПараметрыСоединенияСКластером.ПарольАгента);
	//
	...
	Для Каждого Кластер Из АдминистрированиеСервера.ПолучитьКластеры() Цикл
		...
		Кластер.ВыполнитьАутентификацию(ПараметрыСоединенияСКластером.ЛогинКластера,
			ПараметрыСоединенияСКластером.ПарольКластера);
		//
		Возврат Кластер;
	КонецЦикла;
...
&НаСервереБезКонтекста
Функция ОбъектИнформационнойБазы(Знач ПараметрыСоединенияСКластером, Знач СоединениеСКластером,
	Знач ПараметрыСоединенияСИБ, Знач ИмяИБ)
...
&НаСервереБезКонтекста
Функция ДанныеДляИзмененияСуществующейИБ(Знач ПараметрыСоединенияСКластером, Знач ПараметрыСоединенияСИБ,
	Знач ИмяИнформационнойБазы, Знач ВерсияПлатформы)

	СоединениеСКластером = СоединениеСКластером(ПараметрыСоединенияСКластером, Истина);
	ИБ = ОбъектИнформационнойБазы(ПараметрыСоединенияСКластером, СоединениеСКластером,
		ПараметрыСоединенияСИБ, ИмяИнформационнойБазы);
	//
	...
	Если ПараметрыСоединенияСКластером.МетодПодключения = "COM" Тогда
		...
		Если ВерсияПлатформы >= 3.24 Тогда
			ОписаниеИБ.ЗапретитьЛокальноеРаспознаваниеРечи = ИБ.DisableLocalSpeechToText;
		КонецЕсли;
		
		Если ВерсияПлатформы >= 3.25 Тогда
			ОписаниеИБ.ЗадержкаВыгрузкиКонфигурацииРабочимПроцессом
				= ИБ.ConfigurationUnloadDelayByWorkingProcessWithoutActiveUsers;
			//
			ОписаниеИБ.МинимальныйПериодЗапускаРегламентныхЗаданий = ИБ.MinScheduledJobsStartPeriodWithoutActiveUsers;
			ОписаниеИБ.МаксимальныйСдвигЗапускаРегламентныхЗаданий = ИБ.MaxScheduledJobsStartShiftWithoutActiveUsers;
		КонецЕсли;
	...
	ИначеЕсли ПараметрыСоединенияСКластером.МетодПодключения = "RAS" Тогда
		...
		Если ВерсияПлатформы >= 3.24 Тогда
			ОписаниеИБ.ЗапретитьЛокальноеРаспознаваниеРечи = ИБ.ЗапретитьЛокальноеРаспознаваниеРечи;
		КонецЕсли;
		
		Если ВерсияПлатформы >= 3.25 Тогда
			ОписаниеИБ.ЗадержкаВыгрузкиКонфигурацииРабочимПроцессом
				= ИБ.ЗадержкаВыгрузкиКонфигурацииРабочимПроцессомБезАктивныхПользователей;
			//
			ОписаниеИБ.МинимальныйПериодЗапускаРегламентныхЗаданий
				= ИБ.МинимальныйПериодЗапускаРегламентныхЗаданийБезАктивныхПользователей;
			//
			ОписаниеИБ.МаксимальныйСдвигЗапускаРегламентныхЗаданий
				= ИБ.МаксимальныйСдвигЗапускаРегламентныхЗаданийБезАктивныхПользователей;
			//
		КонецЕсли;
...
Функция ЗначениеЗащищенногоСоединенияИзRAS(УровеньБезопасностиСоединений)
	...
	Соответствие.Вставить(АдминистрированиеУровеньБезопасностиСоединений.ЗащищенноеПриУстановкеСоединения,
		"ПриУстановкеСоединения");
	//
...
Функция ЗначениеЗащищенногоСоединенияВRAS(УровеньБезопасностиСоединений)
	...
	Соответствие.Вставить("ПриУстановкеСоединения",
		АдминистрированиеУровеньБезопасностиСоединений.ЗащищенноеПриУстановкеСоединения);
	//
...
&НаКлиенте
Асинх Процедура ПослеВводаПараметровАдминистрированияКластера(НовыеПараметрыСоединенияСКластером,
	...
	Исключение
		...
		Ждать ПредупреждениеАсинх(ТекстОшибки,, НСтр("ru='Ошибка подключения к кластеру'"));
		НачатьАвторизациюВКластере();
	КонецПопытки;
...
//  * Действие - Строка - "Изменить", "Скопировать" или "Удалить"
&НаКлиенте
Асинх Процедура ПослеВводаПараметровСоединенияСИБ(ПараметрыСоединенияСИБ, Контекст) Экспорт
	...
	Если Контекст.Действие = "Изменить" Или Контекст.Действие = "Скопировать" Тогда
		...
		Попытка
			ДанныеДляИзмененияИБ = ДанныеДляИзмененияСуществующейИБ(ПараметрыСоединенияСКластером,
				ПараметрыСоединенияСИБ, Контекст.ИмяИБ, Объект.ВерсияПлатформы);
			//
		...
		Исключение
			ТекстОшибки = ОбработкаОшибок.КраткоеПредставлениеОшибки(ИнформацияОбОшибке());
			Ждать ПредупреждениеАсинх(ТекстОшибки,, НСтр("ru='Ошибка получения информации об информационной базе'"));
		...
		Если ЭтоНовая Тогда
			...
			Если СохраненныеДанныеСУБД <> Неопределено Тогда
				ЗаполнитьЗначенияСвойств(ДанныеДляИзмененияИБ.ОписаниеИБ, СохраненныеДанныеСУБД);
			КонецЕсли;
		КонецЕсли;
		...
		ПараметрыФормы = Новый Структура("ОписаниеИнформационнойБазы, ЭтоНовая, ИменаПрофилейБезопасности",
		ДанныеДляИзмененияИБ.ОписаниеИБ, ЭтоНовая, ДанныеДляИзмененияИБ.ИменаПрофилейБезопасности);
	...
	Иначе
		ВызватьИсключение "Недопустимое действие";
	КонецЕсли;
КонецПроцедуры
...
Асинх Процедура ВызватьУдалениеИБАсинх(ИмяИБ, ПараметрыСоединенияСИБ)
	...
	Исключение
		...
		// BSLLS:LineLength-on
		ТекстОшибки = ОбработкаОшибок.КраткоеПредставлениеОшибки(ИнформацияОбОшибке());
		Ждать ПредупреждениеАсинх(ШапкаПредупреждения + Символы.ПС + Символы.ПС + ТекстОшибки);
	КонецПопытки;
...
&НаСервереБезКонтекста
Процедура УдалитьИБНаСервере(Знач ПараметрыСоединенияСКластером, Знач ИмяИБ,
	Знач ПараметрыСоединенияСИБ, Знач ДействиеСБазойДанных)
	...
	ИначеЕсли ПараметрыСоединенияСКластером.МетодПодключения = "RAS" Тогда
		...
		СоответствиеРежимаУдаленияБД.Вставить("Оставить",
			АдминистрированиеРежимУдаленияИнформационнойБазы.НеВыполнятьДействийСБазойДанных);
		//
		СоответствиеРежимаУдаленияБД.Вставить("Очистить",
...
Асинх Процедура ПослеЗакрытияФормыИнформационнойБазы(ОписаниеИнформационнойБазы, Контекст) Экспорт
	...
	Если Контекст.ЭтоНовая Тогда
		Состояние(НСтр("ru='Подождите...'"),, НСтр("ru='Создание информационной базы'"));
	Иначе
		Состояние(НСтр("ru='Подождите...'"),, НСтр("ru='Сохранение параметров информационной базы'"));
	КонецЕсли;

	Попытка
		ЗаписатьИнформационнуюБазу(ПараметрыСоединенияСКластером, Контекст.ПараметрыСоединенияСИБ,
			ОписаниеИнформационнойБазы, Контекст.ЭтоНовая, Объект.ВерсияПлатформы);
		//
	Исключение
		ТекстОшибки = ОбработкаОшибок.КраткоеПредставлениеОшибки(ИнформацияОбОшибке());
		Ждать ПредупреждениеАсинх(ТекстОшибки,, НСтр("ru='Ошибка при записи информационной базы'"));
		
		ОповещениеОЗавершении = Новый ОписаниеОповещения("ПослеЗакрытияФормыИнформационнойБазы", ЭтотОбъект, Контекст);
		
		ПараметрыФормы = Новый Структура;
		ПараметрыФормы.Вставить("ОписаниеИнформационнойБазы", ОписаниеИнформационнойБазы);
		ПараметрыФормы.Вставить("ЭтоНовая", Контекст.ЭтоНовая);
		ОткрытьФорму(Объект.ИмяМетаданныхОбработки + ".Форма.АдминистрированиеСервера_ИБ",
			ПараметрыФормы,,,,, ОповещениеОЗавершении);
		//
		Возврат;
	...
	Если Контекст.ЭтоНовая Тогда
		ДанныеСУБД = Новый Структура("ТипСУБД, ЛогинСУБД, ПарольСУБД, СерверСУБД");
		ЗаполнитьЗначенияСвойств(ДанныеСУБД, ОписаниеИнформационнойБазы);
		// BSLLS:UnusedLocalVariable-off ложное срабатывание на реквизит формы
		СохраненныеДанныеСУБД = Новый ФиксированнаяСтруктура(ДанныеСУБД);
...
&НаСервереБезКонтекста
Процедура ЗаписатьИнформационнуюБазу(Знач ПараметрыСоединенияСКластером, Знач ПараметрыСоединенияСИБ,
	Знач ОписаниеИнформационнойБазы, Знач ЭтоНовая, Знач ВерсияПлатформы)
	...
	Если Не ЭтоНовая Тогда
		ИБ = ОбъектИнформационнойБазы(ПараметрыСоединенияСКластером, СоединениеСКластером,
			ПараметрыСоединенияСИБ, ОписаниеИнформационнойБазы.Имя);
		//
	КонецЕсли;
	
	Если ПараметрыСоединенияСКластером.МетодПодключения = "COM" Тогда
		ЗаписатьИнформационнуюБазуCOM(СоединениеСКластером, ОписаниеИнформационнойБазы, ЭтоНовая, ИБ, ВерсияПлатформы);
	ИначеЕсли ПараметрыСоединенияСКластером.МетодПодключения = "RAS" Тогда
		ЗаписатьИнформационнуюБазуRAS(СоединениеСКластером, ОписаниеИнформационнойБазы, ЭтоНовая, ИБ, ВерсияПлатформы);
	Иначе
		ВызватьИсключение "Некорректный метод подключения " + ПараметрыСоединенияСКластером.МетодПодключения;
	КонецЕсли;
...
&НаСервереБезКонтекста
Процедура ЗаписатьИнформационнуюБазуCOM(СоединениеСКластером, ОписаниеИнформационнойБазы,
	ЭтоНовая, ИБ, ВерсияПлатформы)
	...
	Иначе
		...
		Если ВерсияПлатформы >= 3.24 Тогда
			ИБ.DisableLocalSpeechToText = ОписаниеИнформационнойБазы.ЗапретитьЛокальноеРаспознаваниеРечи;
		КонецЕсли;
		
		Если ВерсияПлатформы >= 3.25 Тогда
			ИБ.ConfigurationUnloadDelayByWorkingProcessWithoutActiveUsers
				= ОписаниеИнформационнойБазы.ЗадержкаВыгрузкиКонфигурацииРабочимПроцессом;
			//
			ИБ.MinScheduledJobsStartPeriodWithoutActiveUsers
				= ОписаниеИнформационнойБазы.МинимальныйПериодЗапускаРегламентныхЗаданий;
			//
			ИБ.MaxScheduledJobsStartShiftWithoutActiveUsers
				= ОписаниеИнформационнойБазы.МаксимальныйСдвигЗапускаРегламентныхЗаданий;
			//
		КонецЕсли;
...
&НаСервереБезКонтекста
Процедура ЗаписатьИнформационнуюБазуRAS(СоединениеСКластером, ОписаниеИнформационнойБазы,
	ЭтоНовая, ИБ, ВерсияПлатформы)
	...
	Иначе
		...
		Если ВерсияПлатформы >= 3.24 Тогда
			ИБ.ЗапретитьЛокальноеРаспознаваниеРечи = ОписаниеИнформационнойБазы.ЗапретитьЛокальноеРаспознаваниеРечи;
		КонецЕсли;
...
Асинх Процедура УдалитьСеансыАсинх(Сеансы)
	...
	Для Каждого Сеанс Из Сеансы Цикл
		Шаблон = НСтр("ru='Информационная база: %1, сеанс №%2'");
		ПредставленияСеансов.Добавить(СтрШаблон(Шаблон, Сеанс.ИмяИБ, Сеанс.НомерСеанса));
	КонецЦикла;
	
	ШапкаВопроса = НСтр("ru='Следующие сеансы будут завершены. Продолжить?'");
	Ответ = Ждать ВопросАсинх(ШапкаВопроса + Символы.ПС + Символы.ПС + СтрСоединить(ПредставленияСеансов, Символы.ПС),
...
Процедура УдалитьСеансыRAS(СоединениеСКластером, Сеансы, Сообщение)
	...
	Для Каждого Сеанс Из СоединениеСКластером.ПолучитьСеансы() Цикл
		СеансИзСписка = Ложь;
		Для Каждого СтрСеансы Из Сеансы Цикл
			Если Сеанс.ИдентификаторИнформационнойБазы = СтрСеансы.ИдентификаторИБ
				И Сеанс.НомерСеанса = СтрСеансы.НомерСеанса Тогда
...
Процедура НачатьАвторизациюВКластере()
	#Если ВебКлиент Тогда
		Доступность = Истина; // Доступность отключается в ПриОткрытии()
	#КонецЕсли
	...
	ОткрытьФорму(Объект.ИмяМетаданныхОбработки + ".Форма.АдминистрированиеСервера_АвторизацияВКластере",
	ПараметрыФормы,,,,, ОповещениеОЗавершении);
КонецПроцедуры
