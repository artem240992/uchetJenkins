...
Процедура ОбработкаПроверкиЗаполненияНаСервере(Отказ, ПроверяемыеРеквизиты)
	...
	КонецЕсли;
	Если ВидТелаЗапроса = "ДвоичныеДанные" Тогда
...
Процедура СгенерироватьКод(Команда)
	...
	ОсновнойШаблон = СтрЗаменить(ОсновнойШаблон, "[ЗащищенноеСоединение]",
	...
	ОсновнойШаблон = СтрЗаменить(ОсновнойШаблон, "[ИмяПользователя]",
		?(ЗначениеЗаполнено(ИмяПользователя), """" + СтрЗаменить(ИмяПользователя, """", """""") + """", "Неопределено"));
	//
	ОсновнойШаблон = СтрЗаменить(ОсновнойШаблон, "[Пароль]",
		?(ЗначениеЗаполнено(Пароль), """" + СтрЗаменить(Пароль, """", """""") + """", "Неопределено"));
	//
...
Процедура УМ_Сохранение_ПриЗаполненииСохраняемогоОбъектаПереопределяемый(Фабрика, Пакет)
	...
	Если ВидТелаЗапроса = "Строка" Тогда
		Пакет.ТелоЗапросаСтрока = ТелоЗапросаСтрока;
	Иначе
		Пакет.ТелоЗапросаСтрока = "";
	КонецЕсли;

	Если ВидТелаЗапроса = "ДвоичныеДанные" Тогда
		Пакет.ТелоЗапросаДвоичныеДанные = ПолучитьИзВременногоХранилища(ТелоЗапросаДвоичныеДанные);
	Иначе
		Пакет.ТелоЗапросаДвоичныеДанные = Новый ХранилищеЗначения(Неопределено);
	КонецЕсли;
КонецПроцедуры
...
Процедура УМ_Сохранение_ПриЧтенииОбъектаПереопределяемый(Пакет, ВерсияФормата)
	...
	Если ВерсияФормата >= 4 Тогда
		...
		КонецЕсли;
		Если Пакет.ВидТелаЗапроса = "ДвоичныеДанные" Тогда
...
&НаСервереБезКонтекста
Функция ВыполнитьHTTPЗапросНаСервере(Знач СтруктураЗапроса)
...
Функция ВыполнитьHTTPЗапросНаКлиентеНаСервере(СтруктураЗапроса, НакопленноеВремяВыполнения = 0)
	...
	КонецЕсли;
	Если СтруктураЗапроса.ВидТелаЗапроса = "ДвоичныеДанные" Тогда
...
Функция ОсновнойШаблонГенерацииКода()
	// BSLLS:LineLength-off (неделимый шаблон программного кода для вывода пользователю)
	Возврат "HTTPСоединение = Новый HTTPСоединение(""[ИмяСервера]"", [Порт], [ИмяПользователя], [Пароль],, [Таймаут], [ЗащищенноеСоединение]);
...
Процедура УстановитьТекущуюСтраницуТелаЗапроса(Форма)
	Страница = Форма.Элементы.Найти("СтраницаТелоЗапроса" + Форма.ВидТелаЗапроса);
	Форма.Элементы.СтраницыТелоЗапроса.ТекущаяСтраница = Страница;
КонецПроцедуры
