...
Процедура УМ_КрасивыйРедактор_ПриОпределенииРедакторовПереопределяемый(Редакторы)
	...
	Редактор.Вставить("ИмяРеквизита", "консолькодаТекст");
	Редактор.Вставить("ИмяЭлементаТекстовыйДокумент", Элементы.консолькодаТекст.Имя);
		...
		#Иначе
			Для консолькодаСч = 1 По консолькодаКоличествоИтераций Цикл
				...
				Исключение
					...
					ПоказатьПредупреждение(, ТекстОшибки,, "Ошибка выполнения кода");
				КонецПопытки;
			КонецЦикла;
		#КонецЕсли
	Иначе
