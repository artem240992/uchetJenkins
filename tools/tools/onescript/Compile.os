...
Лог.Информация("Сборка артефактов");
...
Иначе
	...
	//Попытка
	//	ПутьКМинимальнойВерсииПлатформы = УправлениеКонфигуратором.ПолучитьПутьКВерсииПлатформы();
	//	Сообщить("ПутьКМинимальнойВерсииПлатформы="+ПутьКМинимальнойВерсииПлатформы);
	//Исключение
	...
	Если ПродолжитьРаботу Тогда
		//УправлениеКонфигуратором.ИспользоватьВерсиюПлатформы();
